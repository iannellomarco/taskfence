...
Install the pinned CLI with `npm install --global https://github.com/iannellomarco/taskfence/archive/refs/tags/v0.1.2.tar.gz`, or build it from the source checkout as described in the [README](../README.md#install-from-source). The CLI supports the same surface for every adapter:
...
By default Claude Code stores native plans under `${CLAUDE_CONFIG_DIR:-~/.claude}/plans`. TaskFence 0.1.2 defers only that default host-managed `Write`, only while `permission_mode` is `plan`, to Claude Code's own gate and never treats it as project authority. On the first write, the destination must not exist: TaskFence creates it exclusively with mode `0600`, durably binds its path to the root host session, and correlates the exact write call and content before deferring. Safe same-session retries and later revisions may reuse that binding; unbound pre-existing and cross-session destinations fail closed. The directory must resolve physically outside the project, be owned by the current user, and not be group/world-writable. Symlinked, hard-linked, project-contained, wrong-owner, group/world-writable, non-regular, and ambiguous destinations also fail closed. Claude Code's supported `plansDirectory` customization is intentionally unsupported: the pre-write hook does not expose the host's resolved setting, so accepting another model-supplied path would weaken the boundary. Customized plan paths fail closed; use the default directory for TaskFence setup. Content remains bounded, and ordinary writes still require an active contract.

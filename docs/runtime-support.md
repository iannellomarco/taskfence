...
Install the pinned CLI with `npm install --global https://github.com/iannellomarco/taskfence/archive/refs/tags/v0.1.3.tar.gz`, or build it from the source checkout as described in the [README](../README.md#install-from-source). The CLI supports the same surface for every adapter:
...
By default Claude Code stores native plans under `${CLAUDE_CONFIG_DIR:-~/.claude}/plans`. TaskFence 0.1.3 defers only that default host-managed `Write`, only while `permission_mode` is `plan`, to Claude Code's own gate and never treats it as project authority. On the first write, the destination must not exist: TaskFence atomically records a durable claim in a safe hidden directory on the same plans filesystem, using the target basename so the filesystem itself arbitrates case and Unicode aliases. That claim binds the canonical project root, root host session, and path before TaskFence correlates the exact write call and content, while leaving target creation to Claude's `Write` tool. Safe same-session later revisions may reuse the binding only after the preceding file content matches its completed reservation; unbound pre-existing, competing cross-project, cross-session, and filesystem-aliased destinations fail closed. The plans directory must resolve physically outside the project, be owned by the current user, and not be group/world-writable. Symlinked, hard-linked, project-contained, wrong-owner, group/world-writable, non-regular, and ambiguous destinations also fail closed. Claude Code's supported `plansDirectory` customization is intentionally unsupported: the pre-write hook does not expose the host's resolved setting, so accepting another model-supplied path would weaken the boundary. Customized plan paths fail closed; use the default directory for TaskFence setup. Content remains bounded, and ordinary writes still require an active contract.

For `ExitPlanMode`, a root-session pre hook validates the host-injected `tool_input.plan`, optional `planFilePath`, session-bound file when supplied, and the exact contract, then durably correlates its hash with the session and tool-use ID. It returns `ask`, not `allow`, so TaskFence does not bypass the native user approval prompt. Official guidance prefers returned `tool_response.plan`, which TaskFence validates when present. In the tested Claude Code 2.1.220 interactive flow, the successful post payload instead had empty `tool_input`, `tool_response.plan: null`, and `tool_response.filePath`; that exact shape is runtime-observed, not documented as stable. The generated 2.1.220 output schema permits nullable plan text and an optional file path, so TaskFence uses the secure, session-bound plan-file read only as that pinned fallback and verifies it against the pre-hook correlation. The correlation remains durable until activation succeeds; retries resume the exact staged or checkpointing contract, and an exact already-active retry is idempotent. Only then is the correlation removed. A child `ExitPlanMode` is denied.
